...
public:
  ...
  // filename.
  Texture* Get(const std::string& texturename, const std::string& alphaname);
  ...
  Texture* CreateNewTexture(const std::string& bitmapname,
                             const std::string& alphafilename);

  // Load alpha layer file if required. 
  bool LoadAlphaData(Texture* pt, const std::string& alphafilename);
  ...
  bool Save(File* pf, Texture* pTex);
#endif
