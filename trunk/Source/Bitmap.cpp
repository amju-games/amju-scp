  ...
  unsigned int h)
{
  ...
  *(uint32*)(&header.bytes[2]) = 
    (uint32)(sizeof(BitmapFileHeader) + sizeof(BitmapInfo) + w * h * 3);
  ...
  *(uint32*)(&header.bytes[10]) = 
    (uint32)(sizeof(BitmapFileHeader) + sizeof(BitmapInfo) - 4);
  // NB maybe rgb quad is not required so 4 bytes too long.

  BitmapInfo info;
  info.bmiHeader.biSize = (uint32)sizeof(BitmapInfoHeader);
  info.bmiHeader.biWidth = (int32)w;
  info.bmiHeader.biHeight = (int32)h;
  info.bmiHeader.biPlanes = 1;
  info.bmiHeader.biBitCount = (uint16)24;
  info.bmiHeader.biCompression = 0;
  // Assume 3 bytes per pixel
  info.bmiHeader.biSizeImage = (uint32)w * h * 3;
